...
PreProcessingFilter::PreProcessingFilter(int targetWidth, int targetHeight, CascadeClassifier& eyeCascade1, CascadeClassifier& eyeCascade2)
{
...
ImageData* PreProcessingFilter::filter(ImageData* image)
{
    ...
    {
        image->faces[i] = getPreprocessedFace(image->faces[i], eyeCascade1, eyeCascade2, true, &leftEye, &rightEye, &searchedLeftEye, &searchedRightEye);
    }
    return image;
}
...
void PreProcessingFilter::detectLargestObject(const Mat &image, CascadeClassifier &cascade, Rect &largestObject, int scaledWidth)
{
    ...
    Size minFeatureSize = Size(0, 0);
    float searchScaleFactor = 1.01f;
    int minNeighbors = 2;
...
void PreProcessingFilter::detectBothEyes(const Mat &face, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, Point &leftEye, Point &rightEye, Rect *searchedLeftEye, Rect *searchedRightEye)
{
    ...
    /*imshow("tl face", topLeftOfFace);
    imshow("tr face", topRightOfFace);
    waitKey(0);*/
...
Mat PreProcessingFilter::getPreprocessedFace(Mat &srcImg, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, bool doLeftAndRightSeparately, Point *storeLeftEye, Point *storeRightEye, Rect *searchedLeftEye, Rect *searchedRightEye)
{
    int desiredFaceHeight = targetHeight;
    int desiredFaceWidth = targetWidth;
    ...
    if (leftEye.x >= 0 && rightEye.x >= 0)
    {
        ...
        {
            equalizeHist(gray, gray); //used to be warped warped
        }
        else
        {
